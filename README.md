...
Validator commands focus on interactions with consensus validators.

Note that most validator commands should take an additional `--consensus-connection` parameter which connects them to a consensus (beacon) node.  This is used to generate checks prior to carrying out validator operations, and is important to ensure that you do not lose Ether, either by sending transactions that will not be actioned, or by carrying out incorrect operations.  If you are okay with not carrying out these checks you can skip this parameter, and add the `--no-safety-checks` flag, to transmit transactions without these checks.

### `deposit`

`ethereal validator deposit` creates and sends a consensus deposit contract transaction using data generated by `ethdo`.  For example:

```sh
$ ethereal validator deposit --data=deposit.json --eth2network=mainnet --from=0x7E5F4552091A69125d5DfCb7b8C2659029395Bdf
```
...
`ethereal validator exit` creates and sends a transaction to exit an active consensus validator.  For example:
...
`ethereal validator compound` sends a transaction to turn a non-compounding (max 32 ETH) consensus validator into a compounding (max 2048 ETH) consensus validator.  For example:
...
`ethereal validator consolidate` consolidates two consensus validator into a compounding (max 2048 ETH) consensus validator.  For example:
